...
Modeling_testset_C1<-slice(Modeling_C1,round(dim(Modeling_C1)[1]*a)+1:dim(Modeling_C1)[1])

#Fibrosis
Modeling_C2<-filter(Modeling_dataset, Category=="Fibrosis")
Modeling_trainset_C2<-slice(Modeling_C2,1:round(dim(Modeling_C2)[1]*a))
Modeling_testset_C2<-slice(Modeling_C2,round(dim(Modeling_C2)[1]*a)+1:dim(Modeling_C2)[1])
...
```{r}
Model_test_pred <- knn(train = as.matrix(Modeling_trainset_feat), test = as.matrix(Modeling_testset_feat), cl = as.matrix(Modeling_trainset_label), k = 21)
...
```{r}
confusionMatrix(Model_test_pred, Modeling_testset_label[[1]], positive = NULL, dnn = c("Prediction", "True"))
```
